    ...
    while running:
        ...
        for i in events:
            if i.type == pygame.QUIT:
                ...
                pygame_widgets.update(events) # обновляем события и передаем нажатое (введенные буквы) при нажатии Enter
...
def main():
    ...
    while running:
        ...
        if program:
            ...
            short_program = program[-5:]
            for elem in short_program:
                if short_program.index(elem) == 0:
                    last_elem = elem
        ...
        if text_from_consol is not None:
            while HEIGHT // 2 > 260 + (k - 1) * 30 and k < len(text_from_consol):
                screen.blit(text_from_consol[k], (760, HEIGHT // 2 + 250 + k * 30))
                k += 1
